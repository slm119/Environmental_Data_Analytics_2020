...
```{r, message = FALSE}
getwd()
library(plyr) # load plyr first so that dplyr is masking out plyr, what ever is loaded most recently will mask out previous ones
library(tidyverse)
library(lubridate)
...
# Notice that other lake names didn't go away, even though they have zero values need to *drop levels*
...
NTL.nutrient.data.PeterPaul <- 
  NTL.nutrient.data.PeterPaul %>% # specifies the df to use
  mutate(month = month(sampledate)) %>% # add a month column
  select(lakeid:daynum, month, sampledate:comments) %>% # select the columns you want
  drop_na(depth) # drop rows where depth is shown as NA
...
NTL.nutrient.data.PeterPaul.subset <- NTL.nutrient.data.PeterPaul %>%
...
NTL.phys.nutrient.data.PeterPaul <- #full_join is usually the most used
  full_join(NTL.phys.data.PeterPaul.subset,
...
```{r}
# use list.files to read in all files in the same folder - first need to create a list with all the file paths
LitterFiles = list.files(path = "./Data/Raw/NIWO_Litter/", pattern="*.csv", full.names=TRUE)
LitterFiles

# then read in all the listed files, separately
Litter <- LitterFiles %>%
  ldply(read.csv)

# then you could join those files or whatever you wanted
```
...
# left join to add the trap info (which has much less info/smaller dimensions) to the onto the Litter data
...
```{r}
# make a table that summarises all the data for the surface, separated by lake and month of the year
