...
1. sf - the data frame with feature attributes and feature geometries, this contains...
...
#### 1.6.3 stars - for raster and vector data
...
We'll also being using Southeast state boundary data from the United States Census Bureau. Check out the *maps* packages for state boundaries and other open source spatial data.
...
my_tabular_data_raw <- read.csv("./data/tabular_data/conterm_topo.txt")
...
When *setting* a CRS, you're labeling the spatial data with a CRS it already has but is not showing up when you load the data into R. Setting also includes defining a missing ESPG or proj4 string; when one is complete but the other is not. This is like "define projection" in ArcGIS.

When *projecting* a CRS, you're converting the spatial data from one CRS to another. For the sf package, transform is used interchangeably with project. This is akin to "Project" in ArcGIS.

It's **very** important to know the difference between setting and projecting!
...
# define proj4 string and EPSG code -- not sure I have the correct one? before the proj4 showed up by epsg was NA, is the proj4 and epsg below for albers or for whatever our data is in??
# add your code here
...
basins_nf_seplains <- st_set_crs(basins_nf_seplains_raw, my_epsg)
st_crs(basins_nf_seplains)

st_crs(gages_raw) <- my_proj4
gages <- st_set_crs(gages_raw, my_epsg)
st_crs(gages)

st_crs(southeast_state_bounds_raw) <- my_proj4
southeast_state_bounds <- st_set_crs(southeast_state_bounds_raw, my_epsg)
...
# add your code here
head(southeast_state_bounds)
NC_state_bounds <- southeast_state_bounds%>%
  filter(NAME == "North Carolina")%>%
  st_geometry() # this drops all attributes from the dataframe and just keeps the geometry information --why? 

head(NC_state_bounds)

# select watersheds that intersect with NC bounds
NC_watersheds<- basins_nf_seplains%>%
  st_intersection(NC_state_bounds)%>%
  st_geometry()
head(NC_watersheds)

# clip water sheds to NC - not working?
...
ggplot(data = southeast_state_bounds) +
  geom_sf(aes(fill = Shape_Area)) + # fill by attribute
...
ggplot()+
  geom_sf(data = NC_state_bounds) +
  geom_sf(data = NC_watersheds, fill = "lightblue", alpha = .5) +
  geom_sf(data = NC_gages, color = "orange")
dev.off()
```
...
nc_bbox <- nc_state_bounds_geom %>%
  ...
  st_bbox() # creates a bounding box based on the buffered NC geometry
nc_bbox # check

# fix bounding box columns so they match what is needed for the ggmap::get_map() function
nc_bbox_fix <- c(left = nc_bbox[[1]], bottom = nc_bbox[[2]], right = nc_bbox[[3]], top = nc_bbox[[4]]) # brackets specify the position in the list, becuase get_map needs the list labels as left, bottom, right, top

# check result
nc_bbox_fix

# get basemap - using get_map
...
ggmap(nc_basemap) + # use ggmap(basemap name) and ggplot will know that you are using a basemap
...
head(nc_gages)
names(nc_gages) # view column names
...
class(my_tabular_data_raw$STAID)
