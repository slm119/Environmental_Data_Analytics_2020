...
O3plot7 <- ggplot(EPAair, aes(x = Date, y = Ozone)) +
  ...
  geom_text(x = as.Date("2020-01-01"), y = 120, label = "unhealthy (sensitive groups)", hjust = 1, fontface = "bold") + # hjust tells it to justify on the right side of the coordinates
  scale_x_date(limits = as.Date(c("2018-01-01", "2019-12-31")), 
    date_breaks = "2 months", date_labels = "%b %y") +
  ylab(expression("O"[3]* " AQI Value")) + # subscript for O3
  theme(axis.text.x = element_text(angle = 45,  hjust = 1)) # adjust angle of axis text
print(O3plot7)  
...
# install.packages("LaCroixColoR") not available
library(viridis)
...
NvsP2 <-
  ...
  labs(x = expression(paste("Total P (",mu,"g/L)")), y = expression(paste("Total N (",mu,"g/L)")), color = "Depth (m)", shape = "") + # change your legend labels here
  scale_shape_manual(values = c(15, 17)) +
  #scale_color_distiller(palette = "Blues", direction = 1, type = "qual") + # use scale_color_brewer for discrete variables
...
Nutrientplot <-
  ...
  geom_boxplot() +
  labs(x = "", y = expression(paste("Conc.",mu,"g/L")), color = expression(paste("Nutrient (", mu, "g/L)")))+
  scale_y_continuous(expand = c(0, 0)) +
  scale_color_brewer(palette = "YlGnBu") +
  #scale_color_manual(values = c("#7fcdbb", "#41b6c4", "#1d91c0", "#225ea8", "#0c2c84")) +
  #scale_color_viridis(discrete = TRUE, end = 0.8) +
  theme(legend.position = "right")+
  scale_color_discrete(breaks = c("nh34", "no23", "tn_ug", "po4", "tp_ug"), 
                       labels = c("ammonia/ammonium", "nitrate/nitrite", "Total N", "phosphate", "Total P"))
print(Nutrientplot)
```
...
levels(Neonics$Endpoint)

ggplot(Neonics) +
  geom_bar(aes(x = reorder(Endpoint, length)))

ggplot(Neonics) +
  geom_bar(aes(x = forcats::fct_infreq(Endpoint)), color = "dark blue", fill = "blue")+
