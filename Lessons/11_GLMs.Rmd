...
ggplot(Litter.Totals, aes(x= as.factor(plotID), y = dryMass))+
  geom_boxplot()

# Format ANOVA as aov
Litter.Totals.anova <- aov(data = Litter.Totals, dryMass ~ plotID)
summary(Litter.Totals.anova) # at least one plot ID is diff than the others

# Format ANOVA as lm
Litter.Totals.anova2 <- lm(data = Litter.Totals, dryMass ~ plotID)
summary(Litter.Totals.anova2)
# plot ID are factors (0 or 1), so yhat(the mean of dry mass) = intercept(first alphabetical group) + estimate for that plot id. y is a constant, no slope 
...
# Extract groupings for pairwise relationships - creates letter groupings!!
...
> mean litter mass differed significantly among sites at niwot lter (fig 1; ANOVA, F(11,198) = 4.813. p < 0.0001) 
...
#to assess pairwise diffrences in non-parametric
...
```{r}
ggplot(Litter, aes(color=nlcdClass, y = dryMass, x = functionalGroup))+
  geom_boxplot()+
  scale_y_log10()
...
#intercept is flowers in evergreen (first alphabetically in both categories) so if you want the estimate of needles in a shrubscrub, yhat(needles,shrub) = intercept + functionalgrpneedles estimate + clcdclassschurbscrub estimate
...
# interactin is significant, so the affect of of functional group depends on nlcd class
...
#run with interaction so that HSD output is useful
...
Litter.groups <- HSD.test(Litter.anova.2way5, "Litter.interaction", group = TRUE) #group = TRUE give us the letter groupings
Litter.groups
...
Litter$functionalGroup <- reorder(Litter$functionalGroup, Litter$dryMass, FUN = "mean")
levels(Litter$functionalGroup)

Litter.anova.plot <- ggplot(Litter, aes(y = dryMass, x = nlcdClass, color = functionalGroup)) +
  geom_boxplot()+
  scale_y_log10(limits = c(0.01, 120), labels = trans_format("log10", math_format(10^.x)))+
  labs(x= "NLCD Class", y = "Dry Mass (g)", color = "Functional group")+
  stat_summary(aes(x=nlcdClass, y = dryMass, color = functionalGroup), position = position_dodge(0.75), 
               geom = "text", fun.y = max, vjust = -1, size = 3.5,
               label = c("d", "d", "d", "d", "d", "d", "d", "a",
                          "d", "d", "d", "d", "d", "d", "d", "b",
                         "d", "d", "d", "d", "d", "d", "cd", "bc"),
               show.legend = FALSE)+
  scale_color_brewer(type = "qual", palette = 2)

print(Litter.anova.plot) #letters are in teh wrong order??
