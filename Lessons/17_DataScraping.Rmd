...
State <- webpage %>% # go to this webpage
  html_nodes("table:nth-child(20) td:nth-child(1)") %>% # find these node selected using selector gadget
  html_text() # get the html text from those rows
...
# Coerce into a data frame - list all of them together, will squish them together as one data frame
...
# Use *str_replace* to remove non-numeric characters - must be done for individual columns
Estuary$Estuaries.Assessed.mi2 <- str_replace(Estuary$Estuaries.Assessed.mi2,                                                    pattern = "([,])", replacement = "")  
Estuary$Estuaries.Assessed.percent <- str_replace(Estuary$Estuaries.Assessed.percent,pattern = "([%])", replacement = "")
Estuary$Estuaries.Assessed.percent <- str_replace(Estuary$Estuaries.Assessed.percent, pattern = "([*])", replacement = "")
Estuary$Estuaries.Impaired.mi2 <- str_replace(Estuary$Estuaries.Impaired.mi2,pattern = "([,])", replacement = "")  
...
> the more area is assessed the more they find that is impaired, the more you look the more you find
