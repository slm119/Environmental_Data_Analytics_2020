...
summary(TNancova.main) # intercept is Paul lake, the alpha and betas are both listed in the same "estimate" column


# interaction effects
TNancova.interaction <- lm(data = PeterPaul.chem.nutrients, tn_ug ~ lakename * depth)
summary(TNancova.interaction)
# interaction is significant which means that the effect of depth depends on the lake name, so the interaction 
# intercept is paul lake and depth = 0
# lakename peter lake - intercept add the estimate to the the alpha for lakename Peter Lake 228+161
# depth is a beta slope for paul lake 90.89
# lakename Peter:depth - slope is 90.894-35.156
...
TNplot <- ggplot(PeterPaul.chem.nutrients, aes(x = depth, y = tn_ug, color = lakename)) + 
  ...
  scale_color_manual(values = c("darkblue", "darkturquoise"))
print(TNplot)
...
TempTest.mixed <- lme(data = NTL.summertemp,
                     temperature_C ~ year4, 
                     random = ~1|lakename) # random effect is lakename 
...
ggplot(NTL.summertemp, aes(x = year4, y = temperature_C, color = lakename)) +
  ...
  theme(legend.spacing.x = unit(0, "cm")) # squished legend lables closer together
print(NTL.tempmodel)
```

Question: How would you interpret the collective results of your mixed effects model in the context of the study question?

> ANSWER: the mixed effects model showed that there is a significant increase in lake surface temperatures over the study period when accounting for variability by lake. 
