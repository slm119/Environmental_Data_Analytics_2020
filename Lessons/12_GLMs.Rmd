...
getwd()
library(tidyverse)
options(scipen = 4) # tells it to display 4 sigfigs
...
# Correlation - correlation squared = Rsquared (adj)
...
>  Irradiance decreases significantly as depth increases (linear regression; R2 = 0.31, F(1,15449) = 6912, p < 0.0001). For each 1 meter increase in depth, irradiance decreases by 96 (units)
...
par(mfrow = c(2,2), mar=c(1,1,1,1)) #"mar" allows us to print these plots correctly
plot(irradiance.regression)
par(mfrow = c(1,1)) # this resets the par(mfrow) option to show one by one
```
...
PeterPaul.chem.nutrients <- filter(PeterPaul.chem.nutrients, 
                                   irradianceWater != 0 & irradianceWater < 5000) # remove outlier point, and when irradiance = 0, log transformation will cause an error when the variable equals zero, in this case there are only 3 rows where irradiance is equal to 0 so we will just remove these from the data 
irradiance.regression2 <- lm(data = PeterPaul.chem.nutrients, log(irradianceWater) ~ depth) 
summary(irradiance.regression2)
 
#check model fit
par(mfrow = c(2,2), mar=c(1,1,1,1)) # tell R to show all four model fit plots at once
...
irradiancebydepth2 <- 
    ggplot(PeterPaul.chem.nutrients, aes(x = depth, y = irradianceWater)) +
    geom_point(pch = 1, color = "darkorange1") +
    scale_y_log10() +
    geom_smooth(method = 'lm', se = FALSE, color = "black", size = .7, linetype = "dashed")+
  labs(x="Depth (m)", y ="Irradiance in water")
print(irradiancebydepth2)
...
``` {r, fig.height = 3, fig.width = 4}
# regression for the reference lake, paul lake
...
TPplot <- ggplot(subset(PeterPaul.chem.nutrients, lakename == "Paul Lake"), 
  ...
  geom_point() +
  xlim(0, 20)
print(TPplot) # not a great visualization for showing the regression relationship, but the linreg model is pretty good
...
PeterPaulnutrients <- 
  ...
  select(tn_ug:po4) %>%
  na.omit() # need to leave out na's before running corplot
PeterPaulCorr <- cor(PeterPaulnutrients)
corrplot(PeterPaulCorr, method = "ellipse")
corrplot.mixed(PeterPaulCorr, upper = "ellipse") # upper is ellipse, lower shows correlation coef
```
...
step(TPAIC) #stepwise AIC to get to most parsimonious model
TPmodel <- lm(data = Paul.naomit, tp_ug ~ dissolvedOxygen + temperature_C + tn_ug)
summary(TPmodel)

```

DO and temp were found to be significant predictors of total P in paul lake and total nitrogen was not significant but did improve the predictive power of the model and so was left in the model. for each unit increa in DO, to decreases by 0.83 ug others held constant. a one degree increase in temp was associated with a .48 ug decrease in tp