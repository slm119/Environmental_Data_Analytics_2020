...
```{r, fig.height = 3, fig.width = 4}
# histogram summarizing the discharge mean
ggplot(USGS.flow.data) +
  geom_histogram(aes(x = discharge.mean))

# same as above but specifies the bindwidth for the plot to get higher resolution
ggplot(USGS.flow.data) +
...
# specify number of bins in the plot
ggplot(USGS.flow.data, aes(x = discharge.mean)) +
  geom_histogram(binwidth = 10) + 
  scale_x_continuous(limits = c(0, 500))
  
# hist of gage height mean column
ggplot(USGS.flow.data) +
...
```{r, fig.height = 3, fig.width = 4}
# lty is linetype with types specified by the integer numbers
ggplot(USGS.flow.data) +
...
# group gage height mean by categories - "cut width" in groups by 1 
ggplot(USGS.flow.data) +
  geom_boxplot(aes(x = gage.height.mean, y = discharge.mean, group = cut_width(gage.height.mean, 1)))

# violin plot with added quantile lines
ggplot(USGS.flow.data) +
...
> Answer: some variables seem to be skewed, there is a data gap between 1972ish and mid 1985ish


How can multiple options for data exploration inform our understanding of our data?

> Answer: can view distributions of single variables and also relationships betwen variables or even stratified within a variable based on the value of another variable
