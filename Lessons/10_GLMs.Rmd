...
EPAair.subsample <- sample_n(EPAair, 5000) # need to create a random subsample of the data bc shapiro test can only use 5000 rows

# Evaluate assumption of normal distribution
shapiro.test((EPAair.subsample$Ozone)) # null is that data is normal, p is < 0.05 so data is not normally distributed
ggplot(EPAair, aes(x = Ozone)) +
  geom_histogram() 
qqnorm(EPAair$Ozone); qqline(EPAair$Ozone) # QQ plot and also run QQ line

O3.onesample <- t.test(EPAair$Ozone, mu = 50, alternative = "less") # Ha mean is < 50
O3.onesample

Ozone.plot <- ggplot(EPAair, aes(x = Ozone)) +
  geom_density(stat = "count", fill = "gray") + # stat = count, changes x axis to the count
  #geom_density(fill = "gray") +
  geom_vline(xintercept = 50, color = "#238b45", lty = 2, size = 0.9) +
  scale_x_continuous(expand = c(0, 0)) + # specify that the axis start exactly at sero
...
> A one-sample t-test was used to determine whether the mean ozone concentrations for EPA data for 2018-2019 was less than or equal to the 50 ppm air quality standard. T-test showed that mean ozone levels are very likley to be lower than 50 ppm (t-test; t = -57.98, p < 0.0001). However, data did not meet the normality assumption of the t-test. 
"EPA ozone measurements for 2018-2019 were signif lower than the AQI "good" standard of 50 ppm ("")."
...
var.test(EPAair$Ozone ~ EPAair$Year) # test for equal variance
# variance is not the same between the two

ggplot(EPAair, aes(x = Ozone, color = as.factor(Year))) +
  geom_freqpoly()

# Format as a t-test
O3.twosample <- t.test(EPAair$Ozone ~ EPAair$Year) # test null that the diff in means is 0
O3.twosample

O3.twosample$p.value

# Format as a GLM
O3.twosample2 <- lm(EPAair$Ozone ~ EPAair$Year) # ozone conc by year
summary(O3.twosample2) # need to use "summary()" function

#par(mfrow = c(2,2)) # run to see all four model fit plots together, to reverse set to c(1,1)
plot(O3.twosample2) # plot will not work with t test, but will with lm , give residual fitted, normal QQ, and cooks distance, and others
...
reorder(x = EPAair$Year, levels = c("2019", "2018"))

ggplot(EPAair, aes(x= Ozone, group = as.factor(Year), fill = as.factor(Year)))+
  geom_density(alpha = 0.5, stat = "count")+
  labs(x= "Ozone (ppm)", y = "Count", fill = "")

#2
ggplot(EPAair)+
  geom_boxplot(aes(x=as.factor(Year), y = Ozone))+
  labs(x="Year", y = "Ozone Conc (ppm)")+
  stat_summary(x=as.factor(Year), y = Ozone)
