...
```{r, message = FALSE}
# Load package - always want to use "message = FALSE" in the header when knitting this section to avoid errors
library(tidyverse)
```
...
./ forward from current wd
../ backward from current wd
...
> ANSWER: data lives on the usgs website https://waterdata.usgs.gov/nwis, all the selection parameters are given in the readme file, also includes any information needed to interpret codes and column names, also includes name and contact info for the person responsible for the "data assembler"
...
# Rename columns - with colnames function, must rename all the column naems
...
str(USGS.flow.data)
dim(USGS.flow.data) # data dimensions
length(USGS.flow.data)

head(USGS.flow.data)
head(USGS.flow.data, 10) # first ten rows
tail(USGS.flow.data, 5) # last 5 rows
USGS.flow.data[30000:30005, c(3, 8, 14)] # show rows 30000 to 30005 and columns 3 8 and 14

# shows classes of each column
...
> Answer: some were filled with "NA" and some are still blank
...
> Answer: if analysis requires all values, may need to eliminate any rows that have missing values

```{r}
USGS.flow.data.complete <- na.omit(USGS.flow.data) # removes rows where any values are "NA" 
dim(USGS.flow.data)
...
```{r}
# reformat the datetime row the yearmonthday format to get it into one integer string
USGS.flow.data$datetime <- format(USGS.flow.data$datetime, "%y%m%d")

# use a function to tell R that if the date is greater than 181231, add 19 to the beginning of the date, if not add 20 to the begining of the date string
create.early.dates <- (function(d) {
       paste0(ifelse(d > 181231,"19","20"),d)
       })
# use the function to fix the row
USGS.flow.data$datetime <- create.early.dates(USGS.flow.data$datetime)

# format to show the full year
