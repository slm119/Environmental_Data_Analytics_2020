...
vector1[3] # return the third item in vector1

# Matrices ----
matrix1 <- matrix(1:20, nrow = 5,ncol = 4) # create a matrix with values 1 through 20 with 5 rows and 4 columns
matrix1
matrix2 <- matrix(1:20, nrow = 5, ncol = 4, byrow = TRUE) # same as above but enter values by moving across the columns first
matrix2
matrix3 <- matrix(1:20, nrow = 5, ncol = 4, byrow = TRUE, # return after comma continues the line
                  dimnames = list(c("uno", "dos", "tres", "cuatro", "cinco"), 
                                  c("un", "deux", "trois", "quatre"))) # give names for each column and row - rows first, then columns 
matrix3

matrix1[4, ] # return matrix 1 , fourth row
matrix1[ , 3] # return 3rd column
matrix1[c(12, 14)] # return the 12th and 14th terms from matrix 1
matrix1[c(12:14)] # return values 12 through 14
matrix1[2:4, 1:3] # return rows 2 thru 4 and columns 1 thru 3 from m1

cells <- c(1, 26, 24, 68)
rnames <- c("R1", "R2")
cnames <- c("C1", "C2") 
matrix4 <- matrix(cells, nrow = 2, ncol = 2, byrow = TRUE,
  dimnames = list(rnames, cnames)) # build a matrix with values from "cells" and row and col names from vectors
matrix4

# Lists ---- 
list1 <- list(name = "Maria", mynumbers = vector1, mymatrix = matrix1, age = 5.3); list1
list1[[2]] # build a list with several different elements

# Data Frames ----
d <- c(1, 2, 3, 4) # What type of vector? integer/numeric
e <- c("red", "white", "red", NA) # What type of vector? character
f <- c(TRUE, TRUE, TRUE, FALSE) # What type of vector? logic
dataframe1 <- data.frame(d,e,f) # create a dataframe by combining vectors d e and f
names(dataframe1) <- c("ID","Color","Passed"); View(dataframe1) # specify the column names and then view the df

dataframe1[1:2,] # call rows 1 through 2, all columns
dataframe1[c("ID","Passed")] # call columns ID and Passed, and all rows
dataframe1$ID # call just the ID column

```
Question: How do the different types of data appear in the Environment tab?

> Answer: vectors are labeled by their name, and also by character type, and dimensions of the data (i.e. single dimension vector with 6 entries shows [1:6])
...
```{r}
highs_f <- c(44,46,50,40,39,45,52,49,50,50)
lows_f <- c(29,28,27,20,18,15,19,27,31,34)
cnames <- c("lows", "highs")

temp.forecast <- data.frame(lows_f,highs_f)
names(temp.forecast) <- cnames
temp.forecast
```
...
```{r}
# (32°F − 32) × 5/9 
highs_c <- highs_f-32*5/9
lows_c <- lows_f-32*5/9
```

Combine your four vectors into a data frame and add informative column names.

```{r}
col.names <- c("low (deg F)", "high (deg F)", "low (deg C)", "high (deg C)")

durham.temps.10day <- data.frame(lows_f,highs_f, lows_c, highs_c)

names(durham.temps.10day) <- col.names
...
```{r}
summary(durham.temps.10day)
sd(durham.temps.10day$`low (deg F)`)
```
...
> Answer: console tells you when packages have been loaded and whether there are any conflicts in function names between packages
