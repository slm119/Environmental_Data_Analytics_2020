...
#4
ntl_AIC <- lm(temperature_C ~ year4 + daynum + depth, data = ntl_lter.july)
step(ntl_AIC)
...
> Answer: Lake, year, day of the month, and depth are all significant predictors of lake temperatures in July. This model explains 74.17% of the variation in the data. 
...
ntl_july_fig1 <- ggplot(data = ntl_lter.july, aes(x = depth, y = temperature_C))+
  ...
  labs(x = "Depth (m)", y = expression("Temp ("*~degree*C*")"), color = "")+
  ylim(0,35)+
...
(difference between the conditional and marginal r squared)
