...
ggplot()+
  ...
  geom_sf_text(aes(label = Ecoregion), data = ICES_ecoregions_wgs84, 
               alpha = .4)+
  geom_sf(aes(color = log(Value), shape = ), data = cadmium_biota_wgs84, inherit.aes = F, shape = 18, size = .5, alpha = 0.5)+ 
  scale_color_viridis(option = "magma", begin = .2) + 
  coord_sf(xlim=c(-73.7, 51.2), ylim = c(33.6, 82.0), crs = 4326)+
  labs(color = "Cadmium conc.in biota\nln(ug/kg)")+
  theme_minimal()
dev.off()
