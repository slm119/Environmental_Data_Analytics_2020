...
\CommentTok{#4}
\NormalTok{ntl_AIC <-}\StringTok{ }\KeywordTok{lm}\NormalTok{(temperature_C }\OperatorTok{~}\StringTok{ }\NormalTok{year4 }\OperatorTok{+}\StringTok{ }\NormalTok{daynum }\OperatorTok{+}\StringTok{ }\NormalTok{depth, }\DataTypeTok{data =}\NormalTok{ ntl_lter.july)}
...
## 
## Call:
## lm(formula = temperature_C ~ year4 + daynum + depth, data = ntl_lter.july)
## 
## Coefficients:
...
predictors of lake temperatures in July. This model explains 74.17\% of
...
\StringTok{  }\KeywordTok{labs}\NormalTok{(}\DataTypeTok{x =} \StringTok{"Depth (m)"}\NormalTok{, }\DataTypeTok{y =} \KeywordTok{expression}\NormalTok{(}\StringTok{"Temp ("}\OperatorTok{*}\ErrorTok{~}\NormalTok{degree}\OperatorTok{*}\NormalTok{C}\OperatorTok{*}\StringTok{")"}\NormalTok{), }\DataTypeTok{color =} \StringTok{""}\NormalTok{)}\OperatorTok{+}
...
effect (difference between the conditional and marginal r squared)
\end{quote}
...
\begin{quote}
Answer: the mixed model is better because it has a lower AIC score.
\end{quote}
