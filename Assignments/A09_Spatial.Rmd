...
ggplot()+
  ...
  geom_sf(data = ICES_ecoregions_wgs84, alpha = 0.5, inherit.aes = F)+ # how to add two color specifications?
  geom_sf(aes(color = log(Value)), data = cadmium_biota_wgs84, inherit.aes = F, shape = 18, size = .5, alpha = 0.5)+ # want to color or scale by value
  geom_sf_text(aes(label = Ecoregion), data = ICES_ecoregions_wgs84, 
               alpha = .4)+
  scale_color_viridis(option = "magma") + # not doing anything...
